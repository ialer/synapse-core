...
impl JwtService {
    ...
    pub fn verify_token(&self, token: &str) -> AuthResult<Claims> {
        ...
        // 解码 payload
        let payload = base64url_decode(parts[1]).map_err(AuthError::InvalidToken)?;
    ...
    fn create_token(&self, claims: &Claims) -> AuthResult<String> {
        ...
        // Claims payload
        let payload_json =
            serde_json::to_string(claims).map_err(|e| AuthError::InternalError(e.to_string()))?;
...
fn base64url_decode(data: &str) -> Result<Vec<u8>, String> {
    ...
    let bytes: Vec<u8> = data
        .chars()
        .filter_map(|c| alphabet.get(&c).copied())
        .collect();

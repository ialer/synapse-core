...
impl AuthService for MemoryAuthService {
    ...
    async fn check_permission(&self, user_id: &str, permission: &str) -> AuthResult<bool> {
        ...
        let permission =
            crate::rbac::Permission::parse_permission(permission).ok_or_else(|| {
                crate::error::AuthError::InvalidToken(format!("Invalid permission: {}", permission))
            })?;
...
impl DiskAuthService {
    ...
    pub async fn new(jwt_config: JwtConfig, secret: impl Into<String>, storage_path: &str) -> Self {
        ...
        Self { inner, users_file }
    }

    /// 从文件加载用户数据（异步）
    async fn load_users_from_file(
        path: &PathBuf,
...
mod tests {
    ...
    async fn test_token_verification() {
        ...
        let claims = service
            .verify_token(&login_result.access_token)
            .await
            .unwrap();

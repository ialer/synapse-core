...
use crate::mcp::{McpError, McpRequest, McpResponse, McpResult};
...
impl AgentService for MemoryAgentService {
    ...
    async fn handle_mcp_request(&self, session_id: &str, request: McpRequest) -> McpResponse {
        match request {
            ...
            McpRequest::ListTools => McpResponse {
                id: "1".to_string(),
                result: Some(McpResult::ToolList(vec![ToolInfo {
            ...
            McpRequest::SearchData(req) => {
                ...
                {
                    Ok(items) => {
                        let result_items: Vec<crate::mcp::SearchResultItem> = items
                            .into_iter()
                            .map(|i| crate::mcp::SearchResultItem {
                                id: i.id.clone(),
            ...
            McpRequest::GetData(req) => match self.get_with_access(session_id, &req.id).await {
                Ok(item) => McpResponse {
                    id: "1".to_string(),
            ...
            McpRequest::ListResources => McpResponse {
                id: "1".to_string(),
                result: Some(McpResult::ResourceList(vec![crate::mcp::ResourceInfo {
            ...
            McpRequest::UpdateData(req) => {
                ...
                match self.provider.get_data(session_id, &req.id).await {
                    Ok((_id, data_type_str, _tags, _content)) => {
                        let new_content = req.content.map(|c| c.into_bytes()).unwrap_or_default();
                        let new_tags = req.tags.unwrap_or_default();

                        match self
                            .provider
                            .store_data(session_id, &data_type_str, new_content, new_tags)
                            .await
                        {
                            Ok(new_id) => McpResponse {
                                id: "1".to_string(),
                                result: Some(McpResult::OperationResult(
                                    crate::mcp::OperationResult {
                                        success: true,
            ...
            McpRequest::DeleteData(req) => McpResponse {
                ...
                result: Some(McpResult::OperationResult(crate::mcp::OperationResult {
                    success: false,
                    message: format!(
                        "Delete not supported by current DataProvider for id: {}",
                        req.id
                    ),
                    id: None,
                })),
    ...
    async fn get_with_access(
        ...
        let (id, data_type_str, tags, _content) = self
            .provider
...
mod tests {
    ...
    async fn test_search_with_access_no_provider() {
        ...
        let results = service
            .search_with_access(&session_id, "github", Some("credential"), None, 10)
            .await
            .unwrap();
    ...
    async fn test_search_with_access_session_not_found() {
        ...
        let result = service
            .search_with_access("nonexistent-session", "test", None, None, 10)
            .await;
    ...
    async fn test_handle_mcp_request_list_tools() {
        ...
        let response = service_mut
            .handle_mcp_request(&session_id, McpRequest::ListTools)
            .await;
        assert!(response.result.is_some());
        assert!(response.error.is_none());
    ...
    async fn test_handle_mcp_request_list_resources() {
        ...
        let response = service_mut
            .handle_mcp_request(&session_id, McpRequest::ListResources)
            .await;
        assert!(response.result.is_some());
        assert!(response.error.is_none());
    }
    ...
    async fn test_handle_mcp_request_all_variants() {
        ...
        let resp = service
            .handle_mcp_request(
                &session_id,
                McpRequest::Initialize(crate::mcp::InitializeRequest {
                    client_info: crate::mcp::ClientInfo {
                        name: "t".to_string(),
                        version: "1".to_string(),
                    },
        ...
        let resp = service
            .handle_mcp_request(
                &session_id,
                McpRequest::ReadResource(crate::mcp::ReadResourceRequest {
        ...
        let resp = service
            .handle_mcp_request(
                &session_id,
                McpRequest::DeleteData(crate::mcp::DeleteDataRequest {
                    id: "x".to_string(),
                    hard_delete: None,
                }),
            )

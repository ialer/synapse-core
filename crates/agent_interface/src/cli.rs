    ...
    pub fn execute(&mut self, command: CliCommand) -> CliResult {
        ...
        match command {
            CliCommand::Search {
                ...
                success: true,
                message: format!(
                    "Searching for '{}' with type {:?} and limit {:?}",
                    query, data_type, limit
                ),
                data: None,
            },
            CliCommand::Get { id } => CliResult {
                success: true,
            ...
            CliCommand::Delete { id, hard } => CliResult {
                success: true,
                message: format!("Deleting data {} (hard: {})", id, hard),
                data: None,
            },
            CliCommand::List { data_type, limit } => CliResult {
                success: true,
                message: format!(
                    "Listing data with type {:?} and limit {:?}",
                    data_type, limit
                ),
                data: None,
            },
            CliCommand::Export { id, format } => CliResult {
                success: true,
                message: format!("Exporting data {} as {}", id, format),
                data: None,
            },
            CliCommand::Import { file } => CliResult {
                success: true,

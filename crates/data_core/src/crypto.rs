...
impl Cipher {
    ...
    pub fn from_password(password: &[u8], salt: &[u8]) -> Result<Self, CipherError> {
        ...
        let prk = ring::hkdf::Prk::new_less_safe(ring::hkdf::HKDF_SHA256, password);
    ...
    pub fn encrypt(&self, plaintext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, CipherError> {
        ...
        let seal_in_place_result =
            self.key
                .seal_in_place_append_tag(nonce, aead::Aad::from(associated_data), &mut in_out);

        seal_in_place_result.map_err(|e| CipherError::EncryptionFailed(e.to_string()))?;
    ...
    pub fn decrypt(&self, ciphertext: &[u8], aad: Option<&[u8]>) -> Result<Vec<u8>, CipherError> {
        ...
        if ciphertext.len() < Self::NONCE_LENGTH + Self::TAG_LENGTH {
            return Err(CipherError::DecryptionFailed("数据长度不足".to_string()));
        }
        ...
        // 执行解密
        let associated_data = aad.unwrap_or(&[]);
        let open_in_place_result =
            self.key
                .open_in_place(nonce, aead::Aad::from(associated_data), &mut in_out);

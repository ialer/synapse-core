//! S3/MinIO 存储后端实现
//! 
//! 基于 opendal 库实现 S3 协议的存储后端。

use async_trait::async_trait;
use opendal::{services::S3, Operator};

use crate::error::{StorageError, StorageResult};
use crate::trait_def::StorageBackend;

/// S3/MinIO 存储后端
pub struct S3Backend {
    /// opendal 操作器
    op: Operator,
    
    /// 服务端点
    endpoint: String,
    
    /// 存储桶
    bucket: String,
}

impl S3Backend {
    /// 创建新的 S3 存储后端
    pub fn new(
        endpoint: &str,
        bucket: &str,
        access_key: &str,
        secret_key: &str,
    ) -> StorageResult<Self> {
        let builder = S3::default()
            .endpoint(endpoint)
            .bucket(bucket)
            .access_key_id(access_key)
            .secret_access_key(secret_key)
            .region("auto");
        
        let op = Operator::new(builder)
            .map_err(|e| StorageError::Config(format!("Failed to create S3 operator: {}", e)))?
            .finish();
        
        Ok(Self {
            op,
            endpoint: endpoint.to_string(),
            bucket: bucket.to_string(),
        })
    }
    
    /// 创建带有自定义区域和根目录的 S3 后端
    pub fn with_config(
        endpoint: &str,
        bucket: &str,
        access_key: &str,
        secret_key: &str,
        region: &str,
        root: &str,
    ) -> StorageResult<Self> {
        let builder = S3::default()
            .endpoint(endpoint)
            .bucket(bucket)
            .access_key_id(access_key)
            .secret_access_key(secret_key)
            .region(region)
            .root(root);
        
        let op = Operator::new(builder)
            .map_err(|e| StorageError::Config(format!("Failed to create S3 operator: {}", e)))?
            .finish();
        
        Ok(Self {
            op,
            endpoint: endpoint.to_string(),
            bucket: bucket.to_string(),
        })
    }
    
    /// 获取服务端点
    pub fn endpoint(&self) -> &str {
        &self.endpoint
    }
    
    /// 获取存储桶名称
    pub fn bucket(&self) -> &str {
        &self.bucket
    }
}

#[async_trait]
impl StorageBackend for S3Backend {
    async fn save(&self, key: &str, data: &[u8]) -> StorageResult<()> {
        let _ = self.op
            .write(key, data.to_vec())
            .await
            .map_err(|e| StorageError::Network(format!("S3 write failed: {}", e)))?;
        Ok(())
    }
    
    async fn load(&self, key: &str) -> StorageResult<Vec<u8>> {
        self.op
            .read(key)
            .await
            .map(|data| data.to_vec())
            .map_err(|e| StorageError::Network(format!("S3 read failed: {}", e)))
    }
    
    async fn list(&self, prefix: &str) -> StorageResult<Vec<String>> {
        let entries = self.op
            .list(prefix)
            .await
            .map_err(|e| StorageError::Network(format!("S3 list failed: {}", e)))?;
        
        let paths: Vec<String> = entries
            .into_iter()
            .map(|entry| entry.path().to_string())
            .collect();
        
        Ok(paths)
    }
    
    async fn delete(&self, key: &str) -> StorageResult<()> {
        self.op
            .delete(key)
            .await
            .map_err(|e| StorageError::Network(format!("S3 delete failed: {}", e)))
    }
    
    async fn exists(&self, key: &str) -> StorageResult<bool> {
        match self.op.stat(key).await {
            Ok(_) => Ok(true),
            Err(e) => {
                if e.to_string().contains("not found") {
                    Ok(false)
                } else {
                    Err(StorageError::Network(format!("S3 exists check failed: {}", e)))
                }
            }
        }
    }
    
    async fn size(&self, key: &str) -> StorageResult<u64> {
        let meta = self.op
            .stat(key)
            .await
            .map_err(|e| StorageError::Network(format!("S3 stat failed: {}", e)))?;
        
        Ok(meta.content_length())
    }
    
    async fn copy(&self, src: &str, dst: &str) -> StorageResult<()> {
        self.op
            .copy(src, dst)
            .await
            .map_err(|e| StorageError::Network(format!("S3 copy failed: {}", e)))
    }
    
    async fn rename(&self, src: &str, dst: &str) -> StorageResult<()> {
        self.op
            .rename(src, dst)
            .await
            .map_err(|e| StorageError::Network(format!("S3 rename failed: {}", e)))
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    
    #[test]
    fn test_s3_backend_creation() {
        let result = S3Backend::new(
            "http://localhost:9000",
            "test-bucket",
            "minioadmin",
            "minio12345",
        );
        assert!(result.is_ok());
    }
    
    #[test]
    fn test_s3_backend_endpoint() {
        let backend = S3Backend::new(
            "http://localhost:9000",
            "test-bucket",
            "minioadmin",
            "minio12345",
        ).unwrap();
        assert_eq!(backend.endpoint(), "http://localhost:9000");
        assert_eq!(backend.bucket(), "test-bucket");
    }
}
